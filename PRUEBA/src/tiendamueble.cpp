...
float numStrirng;
string mueble;
string muebles[4][2] = {{"Sillas", "4.80"}, {"Veladores", "25"}, {"Mesas", "50"}, {"Camas", "100"}};

void mayuscula(string &muebleMAY)
{
    for (int i = 0; i < mueble.length(); i++)
    {
        muebleMAY[i] = towupper(muebleMAY[i]);
    }
}
...
void buscarMuebles()
{
    cout << "ESCRIBA EL MUEBLE A COMPRAR: ";
    cin >> mueble;
    for (int i = 0; i < 4; i++)
    {
        if (mueble == muebles[i][0])
        {
            newi = i;
        }
    }
    numStrirng = stof(muebles[newi][1]);
    mayuscula(mueble);
    cout << "CANTIDAS DE " << mueble << ": ";
    cin >> cantidad;
    total = numStrirng * cantidad;
    totalMuebles += total;
    cout << endl;
}

void agregarMasMueble()
{
    do
    {
        cout << "DESEA COMPRAR ALGO MAS?\n"
             << "1. SI\n"
             << "2. NO\n"
             << "OPCIONES: ";
        cin >> opc;
...
void descuentoMuebles()
{
    // VELADOR
    // if (mueble == muebles[1][0])
    // {
        if (cantidad <= 20)
        {
            descuento = totalMuebles * 0.05;
            totalDescuento = totalMuebles - descuento;
        }
        if (cantidad > 20)
        {
            descuento = totalMuebles * 0.075;
            totalDescuento = totalMuebles - descuento;
        }
        cout << totalDescuento;
    // }
